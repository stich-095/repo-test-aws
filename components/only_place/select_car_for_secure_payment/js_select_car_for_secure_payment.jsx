import "./css_select_car_for_secure_payment.css";
...
export default function SelectCarForSecurePayment({ data_from_api, seler_uid, uid_secure_buy }) {
    ...
    const [selectedCarUid, setSelectedCarUid] = useState(null); 


    const handleSendCarSelected = () => {
        try{
            const response = axiosInstance.patch(
                `secure_payment/handle/${uid_secure_buy}/`,
                {
                    car: selectedCarUid
                },
                
            )
            router.push(`/dashboard/seller/${selerUid}/tool/secure_buy/create/${uid_secure_buy}/select_car/abount_transaction`);
    ...
    const handleCheckboxChange = (uid) => {
        setSelectedCarUid(selectedCarUid === uid ? null : uid); // Marcar o desmarcar el checkbox
      };
    ...
    return (
        <div>
                        ...
                        <div key={item.uid} className="cont_all_product">
                            ...
                            <div className="cont_all_items">
                                ...
                                <div style={{ display: "flex", justifyContent: "center" }}>
                                    ...
                                    <input type="checkbox"
                                        name={`select_car_${item.uid}`}
                                        id={`select_car_${item.uid}`}
                                        checked={selectedCarUid === item.uid} // Verifica si es el checkbox seleccionado
                                        onChange={() => handleCheckboxChange(item.uid)}
                                    />
                                    <label style={{marginLeft:"2vh"}} htmlFor={`select_car_${item.uid}`}>Seleccionar para compra segura</label>
                                </div>
            ...
            <div>
                <button onClick={handleSendCarSelected} > Continuar</button>
